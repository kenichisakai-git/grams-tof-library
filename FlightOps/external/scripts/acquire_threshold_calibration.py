...
def acquire_threshold_calibration(config_file, out_file_prefix, noise_reads=4, dark_reads=4, ext_bias=False):
    print("acquire_threshold_calibration:", config_file, out_file_prefix, noise_reads, dark_reads, ext_bias)
    ...
    outFile = open(out_file_prefix + "_baseline.tsv", "w")
    for portID, slaveID, chipID, channelID in activeChannels:
    ...
    #
    outFile = open(out_file_prefix + "_noise.tsv", "w")
...
def safe_acquire_threshold_calibration(config_file, out_file_prefix, noise_reads=4, dark_reads=4, ext_bias=False):
    try:
        return acquire_threshold_calibration(config_file, out_file_prefix, noise_reads, dark_reads, ext_bias)
...
def main(argv):
    ...
    safe_acquire_threshold_calibration(args.config, args.outFilePrefix, args.noise_reads, args.dark_reads, args.ext_bias)
